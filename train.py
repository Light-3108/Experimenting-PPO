...
def train(args, seeds):
    ...
    # old -> envs.obs_space.shape = (3,64,64)  # (12,64,64)
    rollouts = RolloutStorage(args.num_steps, args.num_processes,   #ata change garnu parla, i.e rollout ma 12 vanda ni (3,4) garera rakhne ho ki?
    ...
    if level_sampler:
        ...
        obs = envs.reset()          #before -> (64,3,64,64)   #after -> [64,12,64,64]
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            # Sample actions
            with torch.no_grad():
                obs_id = rollouts.obs[step]  #[64,12,64,64]   #check yo 12 ata sarda ani paxi 3*4 garda, misalign hola?
        ...
        if (j % args.log_interval == 0 and len(episode_rewards) > 1) or j == num_updates - 1:
            ...
            ##change##
            progress = (j + 1) / num_updates
            elapsed_time = timer() - update_start_time
            eta = elapsed_time / progress - elapsed_time

            print(
                f"[Progress] {j+1}/{num_updates} updates "
                f"({progress*100:.2f}%) | "
