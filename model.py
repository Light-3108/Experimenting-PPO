...
def model_for_env_name(args, env):
    if args.num_frame_stack > 1 and args.env_name in PROCGEN_ENVS:    ###change - 4 ###
        model = Policy3D(
            env.observation_space.shape, env.action_space.n,
            base_kwargs={'recurrent': False, 'hidden_size': args.hidden_size})
    elif args.env_name in PROCGEN_ENVS:
        model = Policy(
            env.observation_space.shape, env.action_space.n,
            arch=args.arch,
            base_kwargs={'recurrent': False, 'hidden_size': args.hidden_size})
...
class Policy3D(nn.Module):
    def __init__(self, obs_shape, num_actions, base_kwargs=None):
        super(Policy3D, self).__init__()
        
        if base_kwargs is None:
            base_kwargs = {}
        
        C, H, W = obs_shape   #[12,64,64]   ###----change 6-----###
        n_stack = 4
        cnn_input_shape = (C // n_stack, n_stack, H, W)


        self.base = Cnn3DBase(cnn_input_shape, **base_kwargs)
        self.dist = Categorical(self.base.output_size, num_actions)
    ...
    def _reshape_input(self, inputs):    #####---- change --- 7 #######
        # inputs shape: (batch_size, C * n_stack, H, W) (64, 12, 64, 64)
        # Reshape to: (batch_size, C, n_stack, H, W) (64, 3, 4, 64, 64)
        batch_size, _, H, W = inputs.shape
        C = inputs.shape[1] // self.n_stack
        return inputs.reshape(batch_size, C, self.n_stack, H, W)
    

    def act(self, inputs, rnn_hxs, masks, deterministic=False):
        # inputs = [64,12,64,64]    #expected = [64,3,4,64,64]
        reshaped_inputs = self._reshape_input(inputs)
        value, actor_features, rnn_hxs = self.base(reshaped_inputs, rnn_hxs, masks)
        dist = self.dist(actor_features)
        if deterministic:
            action = dist.mode()
        else:
    ...
    def get_value(self, inputs, rnn_hxs, masks):
        reshaped_inputs = self._reshape_input(inputs)
        value, _, _ = self.base(reshaped_inputs , rnn_hxs, masks)
        return value
...
class Cnn3DBase(NNBase):
    def __init__(self, obs_shape, hidden_size=256, recurrent=False):
        super(Cnn3DBase, self).__init__(recurrent, hidden_size, hidden_size)
        ...
        for out_channels in [16, 32, 32]:
            ...
            shape = conv_seq.get_output_shape()   #where is get_output_shape() ? 
        ...
        self.linear = nn.Linear(shape[0] * shape[2] * shape[3], hidden_size)
        self.critic_linear = nn.Linear(hidden_size, 1)

        self.train()
    
    def forward(self, inputs, rnn_hxs, masks):
        # The input shape will be (num_processes, channels, stack_size, height, width)
        # The 3D conv expects (num_processes, channels, stack_size, height, width)
        x = self.cnn(inputs)  #already normalized
        
        # The example sums over the temporal dimension, let's do that here too.
        x = x.sum(2)
